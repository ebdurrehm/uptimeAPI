...
server.unifiedServer = function (req, res) {
  ...
  debug('Requesting to this path',path);
  //get query string
  ...
  req.on('end', () => {
    ...
    if(path.indexOf('/public')>-1){
      chooseHandler = server.router.public;
    }
    else {
      chooseHandler = typeof (server.router[path]) !== 'undefined' ? server.router[path] : handlers.notFound;
    }
