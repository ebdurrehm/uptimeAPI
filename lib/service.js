    ...
    log.list(false, (err,logData)=>{
       if(!err && logData){
         logData.forEach(fileName=>{
                  ...
                  log.truncate(logId, (err)=>{
                      if(!err){
                        debug ('\x1b[33m%s\x1b[0m','the data successfully truncated');
                          ...
                          console.log('an error occured while truncating',err);
                      }
                  })
                 } else {
           ...
           console.log('an error occured while listining the data',err);
       }
    })
}
...
//create compressing loop
services.compressingLoop = function(){
    setInterval(function(){
        //check every time
        services.rotateLogData();
    },1000*60*60*24)
}
...
services.init = function(){
    ...
    //re-check checks at the  every second 
    services.loop();
     //re-run compressing function at the  every day 
     services.compressingLoop();
