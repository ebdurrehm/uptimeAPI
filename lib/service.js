...
services.validateCheckData = function (originalCheckData) {
    ...
    console.log(originalCheckData.lastChecked, originalCheckData.checkId, originalCheckData.method, 
...
services.performCheckData = function (originalCheckData) {
    ...
    console.log(parsedUrl, hostname,path)
...
services.checkOutCome = function(originalCheckData, outCome){
    ...
    console.log(newCheckData.lastChecked);
    ...
    data.update('checks', newCheckData.checkId,newCheckData, (err)=>{
        if(!err){
            ...
            services.alertToCostumers(newCheckData);
                  ...
                  console.log({"error":"there is nothing to update"})
              }
        } else{
            console.log({"error":"an error occured during updating processs"})  
        }
    })
             ...
             log.compress(logId, NewId, (err)=>{
                 if(!err){
                  log.truncate(logId, (err)=>{
                      if(!err){
                        console.log('the data successfully truncated');
                      } else {
                          console.log(err);
                      }
                  })
                 } else {
                     console.log('an error occured during compress the data',err)
                 }
             })
         })
       } else {
