...
// get all checks
services.getAllChecks = function (callback) {
            ...
            checkFiles.forEach(check => {
                ...
                data.read('checks', check, (err, orginalCheck) => {
                    if (!err && orginalCheck) {
                        ...
                        services.validateCheckData(originalCheckData);
                    } else {
                        callback({ 'error': "the check data was not found" });
                    }
                })
            });
        } else {
            callback({ "error": "not found any check" })
        }
    })
}
...
services.validateCheckData = function (originalCheckData) {
    ...
    originalCheckData.checkId = typeof (originalCheckData.id) === "string" && originalCheckData.id.length >= 15 ? originalCheckData.id : false;
    ...
    if (
        ...
        callback({ "error": "the provided data isn't corrected" });
    }
}
