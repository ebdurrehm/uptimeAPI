...
handlers._checks.post = function(data, callback){
    ...
    const timeoutSeconds = typeof(data.payload.timeoutSeconds) ===  'number' && data.payload.timeoutSeconds >2?data.payload.timeoutSeconds:false;
  ...
  console.log(data.headers.tokenid);
   //read token data
   if(tokenId){
   crudLib.read('tokens',tokenId,(err,tokenData)=>{
       if(!err && tokenData.exiperedTime>Date.now()){
